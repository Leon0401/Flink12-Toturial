...
public class FlinkSQLTest01 {
	public static void main(String[] args) throws Exception {
		...
		// 4. 将流转注册位为试图
		tableEnv.createTemporaryView("test",waterSensorDS);

		// 5. 使用SQL查询注册过的表
		Table query = tableEnv.sqlQuery("select id,count(ts) as ts_cnt,sum(vc) as vc_sum from test group by id");
